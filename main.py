...
# ===== GLOBALS =====
active_attacks = {}
github_tokens = []          # each entry: {token, username, repo, added_at, added_by}
owners = {}
approved_users = {}
pending_users = {}
attack_counters = {}
...
def validate_github_token(token, remove_on_rate_limit=False):
    """
    Returns (is_valid, info, should_remove)
    should_remove = True only for permanent failures (401, 404, etc.)
    For 403 (rate limit), we keep token but mark as temporarily unavailable.
    """
    try:
        ...
        if rate.core.remaining < 1:
            return False, "Rate limit exhausted (403)", False   # keep, but skip
        return True, user.login, False
    except GithubException as e:
        if e.status == 401:
            return False, "Invalid token (401)", True
        elif e.status == 403:
            return False, "Rate limited (403)", False   # keep, skip temporarily
        elif e.status == 404:
            return False, "Token has no permissions (404)", True
        else:
            return False, f"GitHub error: {e.status}", True   # other errors -> remove
    except Exception as e:
        return False, f"Error: {str(e)[:40]}", True

def auto_remove_expired():
    """Only remove tokens that are permanently invalid (401, 404, etc.)"""
    ...
    for td in github_tokens:
        ...
        if is_valid:
            ...
            # Keep token (e.g., rate limited) but we may mark it as temporarily unavailable
            td['username'] = info  # info might be error message
            valid.append(td)
    if removed > 0:
        github_tokens = valid
...
async def addtoken_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        user_id = update.effective_user.id
        # Allow both owners and approved users to add tokens
        ...
        github_tokens.append({
            ...
            'added_by': user_id   # track who added this token
        })
        save_json('github_tokens.json', github_tokens)
        await update.message.reply_text(
...
async def mytokens_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show tokens added by the user themselves."""
    try:
        user_id = update.effective_user.id
        if not can_attack(user_id):
            await update.message.reply_text("⛔ Access Denied", parse_mode='HTML')
...
async def usertokens_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Owner-only: show per-user token counts."""
    try:
        user_id = update.effective_user.id
        if not is_owner(user_id):
            await update.message.reply_text("⛔ Access Denied", parse_mode='HTML')
            return
        # Build a dict of user -> token list
        user_tokens = {}
        for t in github_tokens:
            added_by = t.get('added_by', 'Unknown')
        ...
        for uid, tokens in user_tokens.items():
            username = "Unknown"
            # try to get username from owners or approved or from token itself
            if str(uid) in owners:
                ...
                # Try to get from first token's username (not accurate but ok)
                if tokens:
...
async def removetoken_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        user_id = update.effective_user.id
        # Only owner or the user who added the token can remove it
        if len(context.args) != 1:
        ...
        for i, t in enumerate(github_tokens):
            if t.get('token') == token:
                # Check permission: owner or the one who added
...
# ============================================================
# ===== ATTACK COMMAND (GUNSHOT BINARY + THEME) =====
# ============================================================
    ...
    try:
        ...
        for i in range(total_tokens):
            ...
            try:
                ...
                # Check if gunshot binary exists
                try:
...
# ============================================================
# ===== STATUS, START, HELP, etc. (GUNSHOT THEMED) =====
# ============================================================
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        elif data == "admin_usertokens":
            await usertokens_cmd(update, context)
...
def main():
    try:
        ...
        # Token commands (available to all approved users)
        app.add_handler(CommandHandler("addtoken", addtoken_cmd))
        app.add_handler(CommandHandler("mytokens", mytokens_cmd))
        app.add_handler(CommandHandler("removetoken", removetoken_cmd))  # owner or owner of token
        ...
        logger.info("🔫 GUNSHOT v3.5 is ONLINE (Binary: gunshot)")
