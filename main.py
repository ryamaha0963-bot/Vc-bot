...
import time
import uuid
import asyncio  # <-- IMPORTANT: Lock ke liye
import traceback
...
# 🔒 LOCKS to prevent double processing (Race Condition)
attack_lock = asyncio.Lock()
addtoken_lock = asyncio.Lock()  # <-- NAYA LOCK ADDTOKEN KE LIYE
...
# ============================================================
# ===== TOKEN COMMANDS (FIXED WITH LOCK) =====
# ============================================================

async def addtoken_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # 🔒 LOCK LAGAYA - Ek baar mein sirf ek addtoken process hogi
    async with addtoken_lock:
        try:
            ...
            # DEDUPLICATION — EXACT MATCH (ab safe hai, kyunki lock hai)
...
async def attack_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # 🔒 LOCK LAGAYA - Ek baar mein sirf ek attack command process hogi
    async with attack_lock:
        try:
            ...
            # Ab attack start karo (taaki doosra command block ho jaye)
            start_attack(ip, port, time_val, user_id)

            # Pehla valid token uthao (auto_remove_expired ne already saare valid kar diye hain)
...
"""
            ...
            # Time ke baad attack finish mark karo
