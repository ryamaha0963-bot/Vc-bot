...
# ============================================================
# ===== TOKEN COMMANDS (with Back button) =====
# ============================================================
...
async def mytokens_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if not my_tokens:
            msg = "📭 You haven't added any tokens yet."
            keyboard = [[InlineKeyboardButton("🔙 Back", callback_data="back_start")]]
            await update.message.reply_text(msg, parse_mode='HTML', reply_markup=InlineKeyboardMarkup(keyboard))
            return
        msg = "<b>🔑 YOUR TOKENS</b>\n\n"
        ...
        for i, t in enumerate(github_tokens):
            if t.get('token') == token:
                if is_owner(user_id) or t.get('added_by') == user_id:
                    ...
                    return
        if found:
            msg = f"✅ Token removed. Remaining: {len(github_tokens)}"
        else:
            msg = "❌ Token not found or you don't have permission."
        keyboard = [[InlineKeyboardButton("🔙 Back", callback_data="back_start")]]
        await update.message.reply_text(msg, parse_mode='HTML', reply_markup=InlineKeyboardMarkup(keyboard))
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {str(e)[:100]}")
...
async def cleartokens_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if len(context.args) == 1 and context.args[0].lower() == "confirm":
            ...
            msg = f"⚠️ Delete ALL {count} tokens? Use <code>/cleartokens confirm</code>"
        keyboard = [[InlineKeyboardButton("🔙 Back", callback_data="back_start")]]
        await update.message.reply_text(msg, parse_mode='HTML', reply_markup=InlineKeyboardMarkup(keyboard))
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {str(e)[:100]}")
...
# ============================================================
# ===== ATTACK COMMAND (with Back button) =====
# ============================================================
...
# ============================================================
# ===== STATUS, START, STOP, HELP, ABOUT, etc. (with Back) =====
# ============================================================
...
async def myid_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = f"<b>🆔 YOUR DIGITAL FINGERPRINT</b>\n\n<code>{update.effective_user.id}</code>\n\n<i>Keep this safe – it's your access key.</i>"
    keyboard = [[InlineKeyboardButton("🔙 Back", callback_data="back_start")]]
    await update.message.reply_text(text, parse_mode='HTML', reply_markup=InlineKeyboardMarkup(keyboard))
...
async def approve_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        msg = f"✅ User <code>{target_id}</code> approved for {days} days."
        keyboard = [[InlineKeyboardButton("🔙 Back", callback_data="back_start")]]
        await update.message.reply_text(msg, parse_mode='HTML', reply_markup=InlineKeyboardMarkup(keyboard))
        try:
...
async def maintenance_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        msg = f"🔧 Maintenance {'ENABLED' if mode == 'on' else 'DISABLED'}."
        keyboard = [[InlineKeyboardButton("🔙 Back", callback_data="back_start")]]
        await update.message.reply_text(msg, parse_mode='HTML', reply_markup=InlineKeyboardMarkup(keyboard))
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {str(e)[:100]}")
...
# ============================================================
# ===== FIXED CALLBACK HANDLER (with Back buttons) =====
# ============================================================
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if data == "attack_help":
            ...
            # Replicate status logic with back button
            if not can_attack(user_id):
            ...
            if not github_tokens:
                ...
                if removed > 0:
                    msg = f"🧹 Removed {removed} expired tokens.\n📭 Vault is now empty."
                keyboard = [[InlineKeyboardButton("🔙 Admin Panel", callback_data="back_admin")]]
                await send_or_edit(msg, reply_markup=InlineKeyboardMarkup(keyboard))
                return
            msg = "<b>🔐 TOKEN VAULT</b>\n\n"
            if removed > 0:
            ...
            msg = "📤 Use <code>/binary_upload</code> to upload the gunshot binary."
            keyboard = [[InlineKeyboardButton("🔙 Admin Panel", callback_data="back_admin")]]
            await send_or_edit(msg, parse_mode='HTML', reply_markup=InlineKeyboardMarkup(keyboard))
...
def main():
    try:
        ...
        logger.info("🔫 GUNSHOT v3.7 – सभी जगह Back Button जोड़ा गया!")
