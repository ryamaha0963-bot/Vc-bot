...
def validate_github_token(token):
    ...
    try:
        g = Github(token)
        user = g.get_user()
        user.login  # This will raise exception if token invalid
...
async def checktokens_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Check all tokens and auto-remove expired ones"""
    user_id = update.effective_user.id
...
async def attack_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Validate and use first valid token
    valid_token = None
    for token_data in github_tokens:
        is_valid, _ = validate_github_token(token_data['token'])
        if is_valid:
            valid_token = token_data
            break
    
    if not valid_token:
        await update.message.reply_text("❌ No valid GitHub tokens found. Use /checktokens to see status.")
        finish_attack()
        return
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/checktokens - Check & clean expired tokens 🔥NEW\n"
...
def main():
    ...
    app.add_handler(CommandHandler("checktokens", checktokens_cmd))  # 🔥 NEW
